...
def trainModel(model, imgIn, jstkOut):
    ...
    print(jstkOut)
    history = model.fit(x=imgIn, y=jstkOut, batch_size=32, epochs=400, verbose=2, callbacks=None, validation_split=0.2, shuffle=True, initial_epoch=0)
...
def visualizeData(history):
    ...
    plt.ylabel('loss')
    plt.xlabel('epoch')
    # plt.legend(history.history.keys, ["tAcc, vAcc, tLoss, vLoss"], loc='upper left')
