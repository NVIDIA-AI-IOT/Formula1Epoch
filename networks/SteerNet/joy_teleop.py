			...
			for mapping in cmd['axis_mappings']:
				if len(joy_state.axes)<=mapping['axis']:
					...
					if drivemode == 0:
						...
						# Infer and set values
						if mapping['axis'] == 1:
							joy_value = 1
						else:
							joy_value = self.thread.globalVar
							# Log joy vals (uncomment to use)
							#valsf = open('/home/nvidia/racecar-ws/src/racecar/racecar/scripts/vals.txt', 'a')
