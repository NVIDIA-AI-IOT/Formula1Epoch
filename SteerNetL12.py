...
def model():
    ...
    x = Convolution2D(8, 3, 3, kernel_regularizer=regularizers.l1_l2(0.005))(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    #Convolution/Pooling Layer 3
    x = Convolution2D(16, 3, 3 , kernel_regularizer=regularizers.l1_l2(0.005))(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    #Flattens into 1D array for Usage in final layer
    merged = Flatten()(x)
    #One final fully connected layer for figuring output values
    x = Dense(128 , kernel_regularizer=regularizers.l1_l2(0.005), activity_regularizer=regularizers.l1_l2(0.005))(merged)
...
def trainModel(model, imgIn, jstkOut):
    ...
    csv = CSVLogger('SteerNetL12.csv', separator='\n', append=True)
