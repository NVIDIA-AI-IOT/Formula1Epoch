...
def model():
    ...
    img = Input(shape = (600, 800, 3), name = 'img')
    #Convolution/Pooling Layer 1
    x = Convolution2D(64, 3, 3, kernel_regularizer=regularizers.l2(0.005))(img)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(4, 4))(x)
    #Convolution/Pooling Layer 2
    x = Convolution2D(64, 5, 5, kernel_regularizer=regularizers.l2(0.005))(x)
    x = Activation('relu')(x)
...
def trainModel(model, imgIn, jstkOut):
    ...
    csv = CSVLogger('SteerNetL2Conv6.csv', separator='\n', append=True)
...
def main():
    ...
    '''imtxt = open('image.txt', 'r+')
    jstxt = open('jstk.txt', 'r+')
    imgAr = eval(imtxt.read())
    jstkAr = eval(jstxt.read())
    imtxt.close()
    jstxt.close()'''
    lidarAr = np.load('lidarT1.npy')
    jstkAr = np.load('jstkT1.npy')
    imgAr = np.load('imageT1.npy')
    #imgAr = helperFunctions.getTrainingData('/media/first/UBUNTU/images/')
    ...
    steerModel = keras.models.load_model('SteerNetL2Conv6.h5')
    #Trains model with the function
    trModel = trainModel(steerModel, np.array(imgAr), jstkAr)
