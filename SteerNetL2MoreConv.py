...
def model():
    ...
    x = Convolution2D(4, 3, 3, kernel_regularizer=regularizers.l2(0.005))(img)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    #Convolution/Pooling Layer 2
    x = Convolution2D(8, 3, 3, kernel_regularizer=regularizers.l2(0.005))(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    #Convolution/Pooling Layer 3
    x = Convolution2D(16, 3, 3 , kernel_regularizer=regularizers.l2(0.005))(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
...
def trainModel(model, imgIn, jstkOut):
    ...
    csv = CSVLogger('SteerNetL2MoreConv.csv', separator='\n', append=True)
